...
const Studenttimetableadd = () => {
  ...
  const { load, err, subject } = useSelector((state) => state.get_subjects);
  ...
  const { l, e, gen_groupids } = useSelector((state) => state.get_genGroupId);
  ...
  const { lod2, erro12, room } = useSelector((state) => state.get_room);
  useEffect(() => {
  ...
  lecturerData.map((data) =>
    data.faculty == "Computing"
      ? Clectureroptions.push({
          value: data.name,
          label: data.name,
          isFixed: true,
        })
      : data.faculty == "Engineering"
      ? Electureroptions.push({
          value: data.name,
          label: data.name,
          isFixed: true,
        })
      : data.faculty == "Business"
      ? Blectureroptions.push({
          value: data.name,
          label: data.name,
          isFixed: true,
        })
      : data.faculty == "Humanity science"
      ? Hlectureroptions.push({
          value: data.name,
          label: data.name,
          isFixed: true,
        })
      : Olectureroptions.push({
          value: data.name,
          label: data.name,
          isFixed: true,
        })
  ...
  lecturerData.map((data) =>
    lectureroptions.push({ value: data.name, label: data.name, isFixed: true })
  );

  const subjectoptions = [{ value: "None", label: "None", isFixed: true }];
  subjectData.map((data) =>
    subjectoptions.push({ value: data, label: data.sub_name, isFixed: true })
  );
  ...
  const workinghoursoptions = [
    ...
    { value: "1.30 PM", label: "1.30 PM", isFixed: true },
    { value: "2.30 PM", label: "1.30 PM", isFixed: true },
    { value: "3.30 PM", label: "3.30 PM", isFixed: true },
    { value: "4.30 PM", label: "4.30 PM", isFixed: true },
    { value: "5.30 PM", label: "5.30 PM", isFixed: true },
  ];
  const roomoptions = [
   
    { value: "A450", label: "A450", isFixed: true },
    { value: "A308", label: "A308", isFixed: true },
    { value: "A401", label: "A401", isFixed: true },
    { value: "A650", label: "A650", isFixed: true },
    { value: "B750", label: "B750", isFixed: true },
    { value: "B450", label: "B450", isFixed: true },
  ];

  const ParallelSessionoptions = [
    { value: "MAD[IT2080]Lecture", label: "MAD[IT2080]Lecture", isFixed: true },
    {
      value: "OOP[IT2030]Tutorial",
      label: "OOP[IT2030]Tutorial",
      isFixed: true,
    },
  ...
  const studentgroupIdOptions = [
    { value: "None", label: "None", isFixed: true },
  ];
  var noOfstudents = 120;
  var timeDuration = 3;

  selectedValueTag == "Practical"
    ? subgroupIds.map((data) =>
        studentgroupIdOptions.push({
          value: data,
  ...
  selectedValueTag == "Practical" ? (noOfstudents = 60) : (noOfstudents = 120);

  selectedValueTag == "Practical" || selectedValueTag == "Lecture"
    ? (timeDuration = 2)
    : selectedValueTag == "Tutorial"
    ? (timeDuration = 1)
    : (timeDuration = 3);
  ...
  const handleSubmit = (e) => {
    ...
    let data = {
      ...
      noOfstudents,
      timeDuration,
      selectedValueWorkingday,
      selectedValueWorkinghour,
      selectedValueroom,
      selectedValueParallelSession,
    };
    console.log(data);
          ...
          <form id="myForm" onSubmit={handleSubmit} autoComplete="off">
            ...
            <Select
              ...
              value={tagoptions.filter((obj) =>
                selectedValueTag.includes(obj.value)
              )}
            />
            ...
            <Select
              ...
              components={animatedComponents}
              isMulti
              options={ParallelSessionoptions}
              onChange={handleParallelSession}
              name="tag"
              value={ParallelSessionoptions.filter((obj) =>
                selectedValueParallelSession.includes(obj.value)
              )}
            />
