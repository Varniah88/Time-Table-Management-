...
const Getstudenttimetable = (groupId) => {
  return async (dispatch) => {
    dispatch({ type: GET_STUDENTTIMETABLE_REQUEST });
    try {
      const timestamp = firebase.firestore.FieldValue.serverTimestamp();
      db.collection("student_timetable").onSnapshot((snapshot) => {
        const tempData = snapshot.docs.filter(
          (res) => res.data().selectedValueGroup.label === groupId
        );
        const saveData = tempData.map((res) => res.data());

        dispatch({
          type: GET_STUDENTTIMETABLE_SUCCESS,
          payload: saveData,
        });
      });
    } catch (err) {
      dispatch({
        type: GET_STUDENTTIMETABLE_FAILURE,
        error: err,
      });
    }
  };
...
export { Addstudenttimetable, Getstudenttimetable };
